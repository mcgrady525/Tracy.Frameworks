...
namespace Tracy.Frameworks.Log
{
    ...
    public class DebugLogManager
    {
        ...
        /// 对列初始化
        /// </summary>
        public static ConcurrentQueue<DebugLogInfo> Queue = new ConcurrentQueue<DebugLogInfo>();
        ...
        public static readonly DebugLogManager Instance = new DebugLogManager();
        ...
        {
            Thread.CurrentThread.Name = "DebugLogManager.Run";
            while (true)
            {
                Send();
            }
        ...
        {
            LogHelper.Monitor(Queue, WriteTask, "DebugLog");
        }, TaskCreationOptions.LongRunning).ContinueWith(t => { var exp = t.Exception; }, TaskContinuationOptions.OnlyOnFaulted);
        ...
        public void Enqueue(DebugLogInfo info)
        {
            //营业中,排队才有意思
        ...
        {
            var temp = new List<DebugLogInfo>();
            while (true)
            {
                DebugLogInfo info;
